	...
	for(long double a = 1; a < 1000; ++a){
		...
		for(long double b = floor(a/sqrtl(3)) + 1; b <= a + EPSILON; ++b){
			...
			for(long double c = floor((sqrtl(4*a*a - 3*b*b) - b)/2) + 1; c <= b + EPSILON; ++c){
				long double A = acosl((b*b + c*c - a*a)/(2*b*c)), B = acosl((c*c + a*a - b*b)/(2*c*a)), C = PI - A - B;
				long double u = 1/sinl(A + PI/3), v = 1/sinl(B + PI/3), w = 1/sinl(C + PI/3);
				...
				long double da = sqrtl((u - ha)*(u - ha) + v*v + 2*(u - ha)*v*cosl(C))/sinl(C);
				long double db = sqrtl(u*u + (v - hb)*(v - hb) + 2*u*(v - hb)*cosl(C))/sinl(C);
				long double dc = sqrtl(u*u + v*v + 2*u*v*cosl(C))/sinl(C);
