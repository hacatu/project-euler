...
inline void moveFleasAt(array<double, 30*30> &board, double m, int64_t i, int64_t j){
...
inline void moveFleas(array<double, 30*30> &board, array<double, 30*30> &old_board, int64_t h, int64_t k, int64_t t){
...
int main(){
	double M = 1;
	array<double, 30*30> board_a;
	array<double, 30*30> board_b;
		...
		for(int64_t k = h + 1; k <= 14; ++k){
			...
			for(int64_t i = 0; i < 30; ++i){
				for(int64_t j = 0; j < 30; ++j){
					productboard_a[30*i + j] *= 1. - board_a[30*i + j];
				}
			}
		}
	}
	...
	for(int64_t h = 0; h <= 14; ++h){
		...
		for(int64_t i = 0; i < 30; ++i){
			for(int64_t j = 0; j < 30; ++j){
				productboard_b[30*i + j] *= 1. - board_a[30*i + j];
			}
		}
	}
	for(int64_t i = 0; i < 30; ++i){
