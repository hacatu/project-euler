...
class PythTree:
	...
	def update(self):
		self.tolerance *= 0.8
		self.ts = [(T[0]*a*PythTree.root + T[1]*a + c, T[0]*a, T[1]*a + c) for T in (PythTree.T_L, PythTree.T_R) for (_, a, c) in self.ts]
	...
	def prune(self):
		self.ts = heapq.nsmallest(8192, ((min(z.real - self.left, self.right - z.real, z.imag - self.bottom, self.top - z.imag), a, c) for (z, a, c) in self.ts), key=fst)
