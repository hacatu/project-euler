...
char straight(const char *cards){
	for(int i = 0; i < 4; ++i){
		if(cards[2*i] != cards[2*i + 2] + 1){
			return 0;
		}
	}
	return 1;
...
char flush(const char *cards){
	for(int i = 1; i < 5; ++i){
		if(cards[2*i - 1] != cards[2*i + 1]){
			return 0;
		}
	}
	return 1;
...
char pair(char *cards){
	for(int i = 1; i < 5; ++i){
		if(cards[2*i - 2] == cards[2*i]){
			swapCards(cards, cards + 2*i - 2);
			swapCards(cards + 2, cards + 2*i);
			return 1;
		}
	}
	return 0;
...
char twoPair(char *cards){
	if(cards[0] == cards[2]){
		if(cards[4] == cards[6]){
			return 1;
		}else if(cards[6] == cards[8]){
			swapCards(cards + 4, cards + 8);
			return 1;
		}
		return 0;
	}else if(cards[2] == cards[4]){
		if(cards[6] == cards[8]){
			swapCards(cards, cards + 4);
			swapCards(cards + 4, cards + 8);
			return 1;
		}
		return 0;
...
char three(char *cards){
	for(int i = 2; i < 5; ++i){
		if(cards[2*i - 4] == cards[2*i - 2] && cards[2*i - 2] == cards[2*i]){
			swapCards(cards, cards + 2*i - 4);
			swapCards(cards+2, cards + 2*i-2);
			swapCards(cards + 4, cards + 2*i);
			return 1;
		}
	}
	return 0;
...
char four(char *cards){
	for(int i = 3; i < 5; ++i){
		if(cards[2*i - 6] == cards[2*i - 4] && cards[2*i - 4] == cards[2*i - 2] && cards[2*i - 2] == cards[2*i]){
			swapCards(cards, cards + 2*i - 6);
			swapCards(cards+2, cards + 2*i-4);
			swapCards(cards+4, cards + 2*i-2);
			swapCards(cards + 6, cards + 2*i);
			return 1;
		}
	}
	return 0;
...
char full(char *cards){
	...
	if(cards[4] == cards[6]){
		swapCards(cards, cards + 6);
		swapCards(cards + 2, cards + 8);
		return 1;
	}
	return 0;
}
...
void printHand(const char *cards){
	for(int i = 0; i < 5; ++i){
		printf("%u%c ", cards[2*i], cards[2*i + 1]);
	}
	puts("");
}

char name2num(char name){
	switch(name){
		case '2':return 0;
		case '3':return 1;
		case '4':return 2;
...
int compareCards(const char *a, const char *b){
	return *b - *a;
}

//the ranking is in base 13: trick high second third fourth fifth
uint64_t rankHand(const char *hand){
	uint64_t rank = 0;
	char cards[10];
	for(int i = 0; i < 5; ++i){
		cards[2*i] = name2num(hand[3*i]);
		cards[2*i + 1] = hand[3*i + 1];
	}
	//printHand(cards);
	qsort(cards, 5, 2*sizeof(char), (__compar_fn_t)compareCards);
	//printHand(cards);
	if(flush(cards)){
		if(straight(cards)){//straight flush
			rank = 8;
		}else{
			rank = 5;
		}
